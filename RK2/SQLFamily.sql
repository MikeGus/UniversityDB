-- =============================================================
-- Сценарий создания демонстрационной базы данных Family (Семья)
-- =============================================================
USE master
GO
...
CREATE TABLE dbo.Person (
  PersonID  INT NOT NULL PRIMARY KEY NONCLUSTERED,	-- ID персоны
  LastName  VARCHAR(15) NOT NULL,					-- Фамилия
  FirstName  VARCHAR(15) NOT NULL,					-- Имя
  SrJr  VARCHAR(3) NULL,							-- Обращение
  MaidenName VARCHAR(15) NULL,						-- Девичья фамилия
  Gender CHAR(1) NOT NULL,							-- Пол
  FatherID INT NULL,								-- ID отца
  MotherID INT NULL,								-- ID матери
  DateOfBirth  DATETIME  NULL,						-- Дата рождения
  DateOfDeath  DATETIME  NULL						-- Дата смерти
);
GO
...
CREATE TABLE dbo.Marriage (
  MarriageID  INT NOT NULL PRIMARY KEY NONCLUSTERED,	-- ID брака
  HusbandID  INT NOT NULL,								-- ID мужа
  WifeID  INT NOT NULL,									-- ID жены
  DateOfWedding DATETIME NULL,							-- Дата заключения брака
  DateOfDivorce DATETIME NULL							-- Дата расторжения брака
)
GO
...
AFTER INSERT, UPDATE
AS 
-- Проверка корректности анкетных данных о родителях
IF UPDATE(FatherID)
BEGIN
-- Проверка: верно ли задан пол отца?
  IF EXISTS(
    SELECT * 
  ...
  BEGIN  
	ROLLBACK
    RAISERROR(N'Неверно задан пол отца',14,1)
    RETURN
  END 
END
IF UPDATE(MotherID)
BEGIN
-- Проверка: верно ли задан пол матери?
  IF EXISTS(
    SELECT * 
  ...
  BEGIN  
	ROLLBACK
    RAISERROR(N'Неверно задан пол матери',14,1)
    RETURN
  END
END
-- Триггер может включать и другие виды проверок, а именно:
-- верно ли задан возраст отца?
-- верно ли задана дата смерти отца?
-- верно ли задан возраст матери?
-- верно ли задана дата смерти матери?
RETURN
GO
-- =============================================================
-- Данные для таблицы Person (Персона)
-- =============================================================
...
GO
-- =============================================================
-- Данные для таблицы Marriage (Брачная пара)
-- =============================================================
